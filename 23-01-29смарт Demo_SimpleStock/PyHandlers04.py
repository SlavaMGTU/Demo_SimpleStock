...
def goods_record_on_start(hashMap, _files=None, _data=None):
    ...
    hashMap.put("fill_barcode", json.dumps({"hint": "Введите штрих-код товара", "default_text": hashMap.get("barcode")}))
    ...
    units = []
    units.append("<Выберите единицу измерения ...>")
    query = select(c for c in ui_global.SW_Units)
    for record in query:
...
def outcome_qty_input(hashMap, _files=None, _data=None):
    if hashMap.get("listener") == "":
        ...
        if getfloat_if_exist(hashMap, "qty") > 0:
            with db_session:
                r = ui_global.SW_Account(qty=getfloat_if_exist(-1 * hashMap, "qty"), sku=int(hashMap.get("nom_id")),
                                         cell=int(hashMap.get("cell_id")))
                commit()
